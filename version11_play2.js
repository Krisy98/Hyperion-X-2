...
function avance_position()
{
	...
	laserE = Math.floor(Math.random()*laserCarre);
		...
		for(l=0; l<= laserE; l++){		////////laser ennemi
			...
			posXLEnnemi[l] = posX1[i] +7 ;
			posYLEnnemi[l] = posY1[i] -20 +rayonCarre*2 ;
			posXL1Ennemi[l] = posX1[i] + rayonCarre*2 - 10 ;
			posYL1Ennemi[l] = posY1[i] -20 + rayonCarre*2 ;	
			}				
		}
...
function animate()	{		
	...
	if(totalTime > limitTime)	//////////////////////GAGNER/////////////////////////////////
	{
		...
		context.fillText("score : " +score,500, 370);
		Direction = Direction.Aucune;
...
{							
	for (i = 0; i< nbBalles; i++) {
		if(posX[i] + r >= posXMob && posX[i] <= posXMob + rayon*2& posY[i] + r >= posYMob 
		&& posY[i] <= posYMob + rayon*2 ){
				pv--;
		}
	...
	for(i=0; i<laserEnnemiMax; i++){
		...
		if(posXL1Ennemi[i] >= posXMob && posXL1Ennemi[i] <= posXMob + rayon*2 && posYL1Ennemi[i] >= posYMob && posYL1Ennemi[i] <= posYMob + rayon*2){
			pv--;
			if(posYL1Ennemi[i] > posYMob+ rayon + 10) posXL1Ennemi[i] = -300;
		}
	}
}
...
function commentaire(){
	...
	else if(pvTest<5) context.fillText("D a n g e r !",417, 630);
}