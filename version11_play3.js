...
function avance_position()
{
	...
	for ( i=0; i<nbCarres; i++) {
		...
		for(l=0; l<= laserE; l++){		////////laser ennemi
			...
			posXLEnnemi[l] = posX1[i]  ;
			posYLEnnemi[l] = posY1[i] -10 +rayonCarre*2 ;
			posXL1Ennemi[l] = posX1[i] + rayonCarre*2 -5 ;
...
function dessine()
{
	...
	var pvTest = Math.floor(pv/(pvmax));	
	...
	context.drawImage(imgpv1, 1050, 604);
		...
		context.drawImage(perdu, 400,200);
			context.font= "20px Verdana, Arial, serif";
			context.fillStyle="#009978";
			context.fillText("Bravo votre score est de " +score +".", 850, 200);
		context.fillText("menu",560, 430);
			window.onclick= function(event){
				var x = event.clientX;							
				var y = event.clientY;
